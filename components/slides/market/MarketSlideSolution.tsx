...
function SolutionIcon({ iconKey }: { iconKey: string }) {
  switch (iconKey) {
    case "marketplace":
      return (
        <svg className="w-10 h-10" viewBox="0 0 40 40" fill="none">
          ...
          />
          <path
            d="M35 26l-5 5m0 0l3 0m-3 0l0-3"
            stroke="currentColor"
            strokeWidth="2"
            strokeLinecap="round"
            strokeLinejoin="round"
    ...
    case "intelligence":
      return (
        <svg className="w-10 h-10" viewBox="0 0 40 40" fill="none">
          ...
          />
          <path
            d="M8 26 C12 18, 16 22, 20 16 C24 10, 28 18, 32 14"
            stroke="currentColor"
            strokeWidth="2.5"
            strokeLinecap="round"
            strokeLinejoin="round"
...
export default function MarketSlideSolution() {
  const content = useMarketContent().solution;
  ...
  return (
    <SlideWrapper>
      ...
      <div className="w-full max-w-6xl mx-auto">
        ...
        >
          <div
            className="hidden md:block absolute top-1/2 left-[15%] right-[15%] h-[1px] -translate-y-1/2 z-0"
            aria-hidden="true"
          >
          ...
          {content.pillars.map((pillar, index) => {
            ...
            return (
              ...
              >
                <div
                  className="absolute top-0 left-4 right-4 h-[2px] rounded-full bg-gradient-to-r from-verdaxis-blue to-brand-green"
                  aria-hidden="true"
                />

                {isCenter ? (
                  <div
                    className="absolute -top-1 left-1/2 -translate-x-1/2 w-32 h-8 bg-verdaxis-blue/10 blur-xl rounded-full pointer-events-none"
                    aria-hidden="true"
                  />
                ) : null}

                <div className="mb-5 p-3 rounded-xl bg-verdaxis-blue/10 border border-verdaxis-blue/15 text-verdaxis-blue">
                  <SolutionIcon iconKey={pillar.iconKey} />
                </div>

                <h3 className="text-xl font-heading font-semibold text-slate-900 mb-3">{pillar.title}</h3>
