...
const features = [
  {
    ...
    description:
      "Every listing shows verified carbon intensity scores so you can compare fuels on sustainability, not just price.",
    icon: (
  ...
  },
  {
    title: "RFQ System",
    description:
      "Send structured requests for quotation to multiple suppliers simultaneously. Compare responses in one view.",
    icon: (
      <svg className="w-6 h-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5}>
  ...
  },
  {
    ...
    description:
      "Price-time priority matching automatically connects compatible bids and asks — no manual negotiation required.",
    icon: (
      <svg className="w-6 h-6" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={1.5}>
...
export default function UserSlideMarketplace() {
  return (
    <SlideWrapper>
      ...
      >
        ...
        <motion.div className="flex items-center gap-4 mb-2" variants={fadeInUp}>
          ...
          <h2 className="font-display text-3xl md:text-4xl font-normal text-slate-900 tracking-tight">
            Browse, List, and Trade Green Fuels
          </h2>
        </motion.div>
        <motion.p className="text-slate-500 text-sm md:text-base mb-8 ml-[20px]" variants={fadeInUp}>
          A live exchange purpose-built for low-carbon marine fuels
        </motion.p>
          ...
          <motion.div className="flex flex-col gap-4" variants={staggerContainer}>
            {features.map((f, i) => (
              ...
              >
                ...
                <div>
                  <h3 className="font-heading text-sm font-semibold text-slate-900 mb-1">{f.title}</h3>
          ...
          <motion.div className="flex flex-col gap-5" variants={fadeInUp}>
            ...
            <div className="rounded-xl bg-white border border-slate-200 shadow-card p-5">
              ...
              <div className="flex flex-wrap gap-2">
                {ports.map((port) => (
                  ...
                  >
                    <svg className="w-3 h-3 text-verdaxis-blue" viewBox="0 0 12 12" fill="currentColor" aria-hidden="true">
            ...
            <div className="rounded-xl bg-gradient-to-r from-verdaxis-blue/8 to-brand-green/8 border border-verdaxis-blue/15 p-5 flex items-center gap-3">
              ...
              <p className="text-sm text-slate-600">
                <span className="font-semibold text-slate-800">Live market data</span> — prices update in real time via SSE
              </p>
            </div>
          </motion.div>
        </div>
